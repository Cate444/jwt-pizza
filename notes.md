...
| View profile page                                   | <DinerDashboard user={user} />   | GET /api/franchise/:userId| userRole WHERE role='franchisee' AND userId=?SELECT id, name FROM franchise WHERE id in (...), SELECT u.id, u.name, u.email FROM userRole ... WHERE ur.objectId=? AND ur.role='franchisee', SELECT s.id, s.name, SUM(oi.price)... FROM store ... WHERE s.franchiseId=?|
| View franchise<br/>(as diner)                       |<DinerDashboard user={user} />    |                   |SELECT objectId FROM userRole WHERE role='franchisee' AND userId=?, SELECT id, name FROM franchise WHERE id in (...), SELECT u.id, u.name, u.email FROM userRole ... WHERE ur.objectId=? AND ur.role='franchisee', SELECT s.id, s.name, SUM(oi.price)... FROM store ... WHERE s.franchiseId=?|
...
| View franchise<br/>(as franchisee)                  |<FranchiseDashboard user={user} />| GET /api/franchise/:userId|              |
| Create a store                                      | <CreateStore />                  | POST /api/franchise/:franchiseId/store|              |
| Close a store                                       | <CloseStore />                   |DELETE /api/franchise/:franchiseId/store/:storeId|              |
...
| View Admin page                                     |<AdminDashboard user={user} />    |GET /api/franchise?page=0&limit=10&name=*|              |
| Create a franchise for t@jwt.com                    | <CreateFranchise />              |POST /api/franchise|              |
| Close the franchise for t@jwt.com                   | <CloseFranchise />               |DELETE /api/franchise/:franchiseId|              |
